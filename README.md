...
12. Every app ships the standard McDermott favicon — the exact theme-aware SVG data-URL <link> tag from application-lockup.md (Favicon section), pasted verbatim in <head>. Never a bespoke icon, letter glyph, or emoji, and never omitted.

13. NEVER a solid red button at rest. The one destructive variant rests pale: --color-pale-orange fill, navy text, 3px --color-error left spine. On hover it commits: --color-error-deep (#C22A2A) fill with WHITE text. Never place it ON a pale fill — in danger zones the pale banner wraps the intro text only and the button sits beside it on the surface. Used at entry points AND on the destructive modal's confirm.

14. Every system component class uses the mws- BEM prefix (mws-btn, mws-btn--primary, mws-btn__icon, mws-stepper__circle, mws-table--system…). Engineering's design-fidelity tests match on these exact selectors — never rename or drop them. State classes stay unprefixed (on, checked, completed, has-error).
```
