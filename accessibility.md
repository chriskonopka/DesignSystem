...
**Non-text contrast (1.4.11) in practice — which border token:** the 3:1 rule applies to any border or fill that is the **only thing marking a control's presence and extent** — text inputs, selects, textareas, unchecked checkboxes and radios, the toggle's off-track. Those use `--border-strong` (`#8F8F99` light / `#6B6BD0` dark), which clears 3:1 against both surface and page in both themes — light is at the floor on purpose (3.20:1 on surface, 3.00:1 on page; v1.12 lightened it from `#85858F` as far as the rule allows), so there is no room to lighten it further; `--border-light` and `--border-button` do **not** (1.2–2.4:1) and must never bound a bare control. Exempt by the standard, so `--border-light`/`--border-button` remain correct there: decorative edges (cards, separators, sheet and panel borders — the surface fill and content identify them) and text-labeled buttons (the label identifies the control). Checkboxes/radios already pass via their 2px `--text-primary` border; focus rings pass via `--focus-ring`. Surface-vs-page separation (card on canvas) has **no WCAG requirement** — it's an aesthetic hierarchy choice, and dark mode's navy-on-navy is deliberately subtle.
