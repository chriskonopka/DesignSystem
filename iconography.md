...
Why the constitution bends here — Regular does not survive the size. A 16px favicon gives the glyph about 13px of usable width (v1.12 removed the disc that had cut it to ~10px); even so, a 2px monoline on the 256 grid is an eighth of a device pixel and greys out, and Bold's counters clog and fill in. Fill is the only weight whose silhouette reads at true size. Every other surface in the system renders icons at 16px or larger *with room around them*, where Regular reads correctly — so the carve-out buys legibility in the one place it is genuinely unavailable, and nowhere else.
