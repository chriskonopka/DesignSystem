...
### 7. Licensed assets only
Icons: **Phosphor only** — never another set (Font Awesome, Material, Heroicons, Lucide, custom icon fonts) or stray SVGs. Type: **Arial (sans) + Georgia (serif)** — the firm's brand-approved fonts, referenced in CSS and never downloaded or embedded (no web fonts, `@font-face`, `@import`, or Google Fonts). Don't substitute OS system faces (SF, Segoe) — they differ in metrics per platform. **Arizona** is the licensed brand font for Creative only and is never used in app builds. Anything else is a licensing or brand violation, not a style choice.
...
**Arial (sans) + Georgia (serif)** — the firm's brand-approved fonts. Referenced in CSS, never downloaded or embedded (no web fonts, `@font-face`, `@import`, or Google Fonts). Don't substitute OS system faces (SF, Segoe); Brand standardized on Arial because those render differently per platform. **Arizona** is Creative-only and never used in app builds.

| Token | Stack | Use |
|---|---|---|
| `--font-sans` | `Arial, Helvetica, sans-serif` | Body, UI, buttons, tables & data |
...
- **Source:** Phosphor Icons **only** (`@phosphor-icons/react`) — never another icon set (Font Awesome, Material, Heroicons, Lucide, custom icon fonts); the firm is licensed for Phosphor alone
...
- Web fonts / `@font-face` / `@import` / Google Fonts — system stacks only (unlicensed)
- Any icon set other than Phosphor (Font Awesome, Material, Heroicons, Lucide, custom icon fonts) — unlicensed
