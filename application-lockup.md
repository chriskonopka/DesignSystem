...
## Favicon — every app ships one, automatically

The favicon is part of the identity system, not an asset to design per app. Every app includes this **exact tag** in `<head>` — it is the official symbol as an inline SVG data URL, so there is no icon file to generate, export, or forget. Theme-aware out of the box: navy in light browser chrome, white in dark, via `prefers-color-scheme` inside the SVG.
...
- **Copy it verbatim.** Never redraw the mark, swap in an emoji or letter glyph, or generate a bespoke per-app icon — the no-bespoke-logos rule applies to the tab bar too.
- Works in Chrome, Edge, and Firefox. Safari versions that ignore SVG favicons fall back to their default — acceptable; do not add a PNG pipeline for it. (If a deployment genuinely requires PNG — PWA manifest, pinned tiles — export the same mark at 32/180/512px from this SVG, never a different drawing.)
- One favicon for all apps is deliberate: apps are functions inside the master brand (that's the divider's whole message), so the tab identifies *McDermott*; the `<title>` identifies the app.
...
- A missing favicon, or a bespoke/per-app one — every app ships the standard theme-aware SVG favicon tag (see Favicon)
