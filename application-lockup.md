...
**Amended (v1.12).** Every app's favicon is a **Phosphor Fill glyph naming the app's domain**, in navy, on the tab strip's own background — looked up in the registry below, never designed per app. The glyph fills the whole 16px tile: no disc, no frame, no inset. Family identity in the tab strip comes from the constant treatment (every McDermott tab is a navy Fill silhouette of the same weight, on the same grid) and from the `<title>`; the glyph's job is findability.

**Deprecated (v1.12):** the navy disc + inset glyph (v1.9–v1.11) → the bare glyph. The disc cost 36 percent of the tile (a ~10px glyph in a 16px favicon) and, rendered at true size, the glyphs were too small to tell apart — which was the whole reason v1.9 added them. The disc's family cue was real but was the wrong trade at 16px.

**Deprecated (v1.9), still the fallback:** the symbol-only favicon shared by every app survives *only* for an app whose domain is not yet registered (see Fallback).

### Why it changed
Chrome drops tab titles at roughly eight open tabs, so at portfolio scale the favicon is the last wayfinding signal standing — and a firm-wide identical mark makes every McDermott tab the same tab. The previous rule optimized for brand at exactly the density where people need to find things; the glyph restores the find, and v1.12 gave it the whole tile so the find actually works at 16px.

### The tag
Paste verbatim into `<head>`. Substitute **only** `GLYPH_PATH`; every other byte is fixed.

```html
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns%3D'http://www.w3.org/2000/svg'%20viewBox%3D'0%200%20256%20256'%3E%3Cstyle%3E.g{fill:%23000042}@media(prefers-color-scheme:dark){.g{fill:%23FFFFFF}}%3C/style%3E%3Cg%20class%3D'g'%3EGLYPH_PATH%3C/g%3E%3C/svg%3E">
```
...
| Glyph | No transform — the path sits on the full grid | Phosphor's own ~32-unit margins are the only inset; about 13px of usable glyph in a 16px favicon (was ~10px inside the disc) |
| Light chrome | navy glyph `#000042` | |
| Dark chrome | **white glyph `#FFFFFF`** | Navy on Chrome's `#202124` strip is **1.21:1** — invisible. The `prefers-color-scheme` rule in the tag handles it; never ship a single-color tag. |
...
- **No frame, ever.** Not a disc, not a rounded square, not a hairline — any container spends tile pixels the glyph needs. If a platform demands a padded shape (PWA maskable icons), that export adds safe-area padding around the *same* glyph; the `<link rel="icon">` tag never does.
- Chrome, Edge and Firefox render it. Safari versions that ignore SVG favicons fall back to their default — acceptable, and not a reason to add a PNG pipeline. (A deployment that genuinely needs PNG — PWA manifest, pinned tiles — exports the same glyph at 32/180/512px, never a different drawing.)
...
- A missing favicon, or a bespoke/hand-drawn per-app one — the tab icon is generated from the registry glyph, never designed (see Favicon)
- A Regular- or Bold-weight favicon glyph — Fill is the only weight that survives 16px
- A disc, rounded square, or any frame around the favicon glyph (deprecated v1.12) — it spends a third of the tile and the glyph stops being tellable apart
- A glyph chosen without adding its registry row, or a near-miss glyph invented to dodge a legitimate collision
- A single-color favicon tag — navy on dark chrome is 1.21:1; the `prefers-color-scheme` swap to white is part of the fixed tag
