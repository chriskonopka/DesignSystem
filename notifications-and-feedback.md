...
## Inline alerts
Inline within a section, full width, same visual treatment as banners (4px left border + pale fill) with smaller padding. Use for contextual warnings, success states, or informational notes within a workflow. The leading icon is 20px and its box is one line of the title tall (`height: calc(16px × 1.5)`, `inline-flex`, centered), so it sits on the first line whether the body wraps or not — the same rule as the field error message in `forms-and-input.md`; no `margin-top` nudges. Field-level error messages are this component at 13pt with a 16px icon.
## Status badges
...
**Labels & tags** (descriptive, non-status: project labels, categories, "Excel (offline)"-style attributes) use the **same pill anatomy** as status badges: `--radius-pill`, pale fill + navy text (`--color-pale-blue` is the default for non-semantic labels) or transparent + 1px `--border-light` for quiet ones. **Never gray-filled rectangular chips, square-cornered outlined boxes, or default-button styling** — a label that looks like a disabled button is off-system. If a label is interactive (click to filter), it's a chip: same pill, plus hover per the interactive-hover rule.
