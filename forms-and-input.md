...
3. **Input** — height `var(--control-h)` (40px), 1px border `var(--border-strong)` (the border is the field's only boundary, so it must meet 3:1 non-text contrast — `--border-light` fails in both themes), `var(--radius)` (2px), `var(--space-3)` (12px) horizontal padding.
...
- **Checkbox / radio** — 24px square (checkbox) or circle (radio), 2px `--text-primary` border (24px matches the WCAG 24×24 minimum target and the showcase implementation; was previously written as 18px — 24 is canon). Checked fill uses `var(--accent-interactive)` — blue in light mode, teal in dark. Check mark is white in light, navy in dark (sufficient contrast on the accent fill in each theme).
