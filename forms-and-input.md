...
3. **Input** — height `var(--control-h)` (40px), 1px border `var(--border-strong)` (the border is the field's only boundary, so it must meet 3:1 non-text contrast — `--border-light` fails in both themes), **`--bg-page` fill** (the tinted interior reads softer than a white cut-out on a white surface; the ring still does the boundary work, so on a `--bg-page` container the field is defined by its border alone and that is fine), `var(--radius)` (2px), `var(--space-3)` (12px) horizontal padding.
4. **Error message** (replaces helper on error) — a compact inline alert, error variant (`notifications-and-feedback.md`): `--color-pale-orange` fill, **4px** `--color-error` left spine (the 3px spine belongs to the destructive button only), `--space-2` `--space-3` padding, 13pt navy text at 1.5 line-height, leading Phosphor `warning-circle` at **16px** (never inherited from the 13pt text — an icon at text size reads as a typo). **Alignment rule for any icon beside text:** the icon's box is exactly one line of the adjacent text tall (`height: calc(font-size × line-height)`, `display: inline-flex; align-items: center`) so it centers on the first line — single-line messages read centered, multi-line messages keep the icon on line one. Never `align-items: center` on the row (a two-line message floats the icon between lines) and never a hand-tuned `margin-top`.
...
| State | Treatment |
|---|---|
| Default | 1px `var(--border-strong)` border |
...
- **Checkbox / radio** — 24px square (checkbox) or circle (radio), **1.5px** `--border-strong` border (browsers snap it to 1 device pixel at 1x and 3 at 2x — both intended; never specify 1px, which reads as a hairline on Retina) and `--bg-page` fill when unchecked — the same functional-boundary token and interior as inputs, so the whole form family sits at one weight (was `--text-primary`, which at 17:1 read heavier than the field beside it; `--border-strong` clears the 3:1 non-text floor and is the lightest legal value). 24px matches the WCAG 24×24 minimum target. Checked: checkbox fills `var(--accent-interactive)` — blue in light mode, teal in dark — with a white check in light, navy in dark; radio keeps the surface fill, and its ring and dot both go `--accent-interactive`.
- **Toggle** — for binary settings that take effect immediately (instant apply, silent success — see `settings-and-profile.md`). Use a checkbox instead if it's part of form submission.
  ...
  - **Track colors:** on = `var(--accent-interactive)` (blue light / teal dark); off = `var(--border-strong)` — the off-track fill is the control's only visible boundary, so it must meet 3:1 non-text contrast (`--border-button` fails in both themes). **Never `--color-navy-gray-*`** — internal-only grays are banned in client-facing UI.
