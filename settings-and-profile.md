...
- **Danger zone:** intro uses the inline-alert error treatment (4px `--color-error` left border, `--color-pale-orange` fill, navy text); actions use the destructive button variant. No pale fill on the section itself — it's a routine surface, not a celebration.
