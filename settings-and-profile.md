...
- **Every app, sidebar or not:** the **account avatar** — last slot of the top-bar session cluster (per `app-shell-and-headers.md`). 32px pill avatar in a 36px icon-button slot, avatar only (no name/caret), `aria-label="Account — <name>"`, tooltip on hover. This is the account's only home.
