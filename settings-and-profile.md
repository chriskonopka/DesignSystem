...
One combined surface for who the user is and how the app behaves for them. Pairs with `app-shell-and-headers.md` (the session cluster, where the account control lives) and `navigation-and-ia.md` (the optional Settings nav row).
...
- **Every app, sidebar or not:** the **account avatar** — last slot of the top-bar session cluster (per `app-shell-and-headers.md`). 32px pill avatar in a 36px icon-button slot, avatar only (no name/caret), `aria-label="Account — <name>"`, tooltip on hover. This is the account's only home. **Deprecated (v1.11):** the pinned sidebar-bottom account control.
- Sidebar apps may additionally list Settings under an "ACCOUNT" nav section (icon marker: `gear`) — optional, a shortcut only; the account menu is always present in the top bar.
...
Popover behavior follows `disclosure-surfaces.md`: max 320px with the viewport-aware cap, opens **below** the avatar and **right-aligned** to it (the control is the last thing in the top bar, so the menu must never overflow the viewport edge), closes on outside click / Escape / selection, returns focus to the trigger. `aria-haspopup="menu"` + `aria-expanded` on the control.
...
- The account avatar stays in the top bar at every width — the session cluster never collapses — so the account is reachable without opening the drawer. The drawer carries no account control.
...
- The account control anywhere but the top-bar session cluster — pinned at the sidebar bottom (deprecated v1.11), in the drawer, or shown in two places
