...
- **One** location indicator on the left — a breadcrumb path or a contextual identifier (record ID, status), **not the page title** (that's the canvas H1; never repeat it in the bar). Empty when there's no path/context to add.
...
- The bar never sits **bare** — the right always carries the session cluster; the left carries a breadcrumb/identifier only when it adds context the canvas title doesn't, and is empty otherwise. If a surface has no chrome at all, omit the top bar entirely rather than rendering a blank strip.
...
3. Identifier badge (e.g., `[Client · Matter]`) — pill, Arial (`--font-sans`), `--bg-page` background with 1px `--border-light` border
...
- Theme toggle is the *session cluster* anchor — app-level, top-bar right, **on every app**. The account avatar joins it **only when there's no sidebar**; sidebar apps pin the account at the sidebar bottom instead (see `navigation-and-ia.md`) — never show the account in both places.
...
- Identifier badge: Arial 12pt, navy text on `var(--bg-page)` background, 1px `var(--border-light)` border, 2px radius
...
- An empty or near-empty top bar — a rendered bar that anchors nothing; populate it or drop the bar
