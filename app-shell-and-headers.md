...
The top bar exists to anchor: where am I, what's saved, what's the primary action. Everything else lives elsewhere. A calm top bar has at most: **one** location indicator, **one** primary action, and a small icon cluster (autosave, theme, account). Anything beyond must earn its slot. But it never sits **bare**: the right always carries the *session cluster* — the fixed-order home for app-level utilities (assistant, notifications, theme toggle, account — see *The session cluster* below). The **theme toggle** and the **account avatar** appear in every app, sidebar or not — the top-bar right is the account's only home (see *The session cluster* below). The left carries only wayfinding the canvas title doesn't provide — a breadcrumb *path* (3+ levels deep) or a contextual identifier (record ID, status) — and is empty otherwise. The one exception is the non-production version slot (below), which renders nothing in Production and nothing in a prototype. **The page title is the canvas H1 and is never repeated in the bar.** If a surface has no chrome at all, omit the top bar entirely; never render a blank strip.
...
| Version/environment label (all environments) | Settings → Support & about (see `settings-and-profile.md`) — never as subtext under the sidebar lockup |
| Running build version (non-production only) | In flow at the end of the top bar's left cluster — see *Non-production version slot* below. Never the session cluster on the right. |
...
| 4 | **Account avatar** — opens the account menu (`settings-and-profile.md`) | Every app with a signed-in user, sidebar or not. Always last in the cluster |

Absent features leave no gap — the cluster compacts. Nothing else joins it: page actions stay in the action area to its left, navigation stays in the sidebar/nav, Settings launches from the account menu (never a top-bar gear), and the sidebar header carries only identity + the collapse control (`navigation-and-ia.md`). Cluster controls are icon-only **ghost buttons** (`mws-btn mws-btn--ghost mws-btn--icon`) — 36×36 minimum targets, `aria-label` each, tooltip on hover. Nothing at rest but the glyph: no border, no fill. Hover and the open state (`aria-expanded="true"`) fill with `--color-navy-gray-1` (`--border-light` dark) and flip the glyph to `--accent-interactive`. The hamburger uses the same variant. Bordered `--secondary` icon buttons are for actions inside content, not app chrome. **Deprecated (v1.12):** bordered secondary icon buttons in the top bar. The account avatar follows the same rule: the 32px pill avatar (initials or photo, per `settings-and-profile.md`) is the whole control, centered in the 36px slot — **no name, role, or caret beside it**. Identity (name + email/role) lives in the account menu's identity header, one click away. The avatar is the only cluster control that is not a Phosphor glyph, which is exactly what makes it findable at a glance.

**Why the top bar, for every shell.** The top bar is the only chrome present in both sidebar and no-sidebar shells (the same reason the version slot lives there), so one placement covers every McDermott app; the control stays visible when the sidebar collapses to the icon rail and when it becomes a mobile drawer; and the session cluster never collapses, so the account is reachable at every width without opening anything first. **Deprecated (v1.11):** pinned account control at the sidebar bottom (avatar + name/role) → the top-bar avatar, all apps. The sidebar bottom is now empty by default (`navigation-and-ia.md`).
...
## Top bar anatomy
**Left (mobile collapse priority — last drops first):** hamburger (<1024px) · app name (often handled by sidebar) · identifier badge · breadcrumb *or* contextual identifier · status indicators. The non-production version slot never drops — it is exempt from this priority order entirely. **Not the page title** — that's the canvas H1; the bar carries the path/context, never a repeat of the title.

**Right (mobile collapse priority — last drops first):** primary action · secondary actions · **session cluster in fixed order: assistant (`sparkle`) · notifications (`bell`) · theme toggle · account avatar**. Only the utilities the app actually has, but never reordered. The **account avatar** is present in every app with a signed-in user, sidebar or not — it is the account's only home, never mirrored in the sidebar (`navigation-and-ia.md`). The session cluster never collapses into the overflow menu — its controls are already icons.
...
## Non-production version slot
Non-production builds (dev, staging, QA) display the running app version. Production never does. The design owns the **slot**; engineering owns the **string** — the prototype ships the slot empty and the build pipeline injects into it at deploy time.

```html
<span class="mws-app-version" data-app-version data-app-env></span>
```

| Concern | Rule |
|---|---|
| Injection target | `data-app-version` — never the class. A restyle must not be able to break the pipeline. |
| Styling | `.mws-app-version`. Covered by the `mws-` classname inventory diff (`governance.md`), so a showcase rebuild cannot drop it silently. |
| Empty state | `:empty { display: none }`. Zero footprint in prototypes and in Production. |
| Filled state | `min-width` reserved for a bare `10.10.10` · `font-variant-numeric: tabular-nums` · `white-space: nowrap`. As the last element in the left cluster it has nothing to its right, so a longer string pushes nothing. |
| Placement | In flow, last element of the top bar's **left** cluster, after the breadcrumb leaf. Never the right-hand session cluster (fixed order, icon-only). The top bar is the only chrome present in both sidebar and no-sidebar shells — that's why it lives there, and a fixed overlay is an anti-pattern: rendered bottom-left it lands on the sidebar account block on desktop and occludes content at phone widths. |
| Type | 11px, `--text-secondary`, ~0.6 opacity at rest, `--bg-surface` fill with a 1px `--border-light` hairline. Quiet — it must not read as a status or an alert. |
| Format | `data-app-env` renders first when present: `QA · 1.0.3`. Bare `1.0.3` when absent. Semantic versioning, no `v` prefix. |
| Accessibility | Not a tab stop. `aria-hidden="true"` and `pointer-events: none` — it is build metadata, not content. |
| Mobile | **Exempt from the ≤640px hide rule** — QA tests at phone widths and needs build identity there too. |

**The slot is never used for the design system's own version.** That lives in exactly two places (`governance.md`, *Versioning*) and this is not one of them.
...
**≤640px:** page title can truncate with ellipsis · primary action label may abbreviate or become icon-only with `aria-label` (but never hides entirely) · hide viewport indicators, environment tags, version labels. The non-production version slot is exempt — it persists at all widths, and only ever renders outside Production anyway.
...
- The account anywhere but the top-bar session cluster — a pinned sidebar profile block (deprecated v1.11), an account row in the nav list, or shown in two places. One home: last slot of the session cluster, every app
- The account avatar carrying a name, role, or caret in the top bar — the avatar alone is the control; identity belongs in the menu header
...
- Bordered or filled icon buttons in the session cluster or as the hamburger — top-bar chrome is the ghost variant; the frame appears only on hover
