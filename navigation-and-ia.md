...
- A pinned user profile / account control at the bottom — **required and consistent when a user is signed in**: avatar + name/role opening the account menu, always at the sidebar bottom. This is the account's home in sidebar apps; the top bar then carries only the theme toggle, not a duplicate avatar (see `app-shell-and-headers.md`).
...
## Breadcrumbs
- Use only for hierarchies 3+ levels deep. Flat hierarchies use the page title (the canvas H1 — no top-bar breadcrumb or title repeat).
