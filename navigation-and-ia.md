...
- **Toggle:** an **icon-only** control in the sidebar header, to the right of the lockup (`margin-left: auto`; the lockup keeps `min-width: 0` so a long app name never pushes the control out) — `sidebar` icon when expanded, `sidebar-simple` when collapsed. No visible label, so `aria-label` + tooltip ("Collapse sidebar" / "Expand sidebar") are required per `iconography.md`. In the collapsed rail the control centers **directly below the lockup symbol**. The same control re-expands. User-driven only, and the state persists per user. Never hover-to-auto-expand, and never a labeled "Collapse" row in the nav list — it reads as a destination.
- **Collapsed rail (72px):** each item shows its **icon only**, centered; labels and section headers hide; the lockup drops to the symbol (see `application-lockup.md`). Active item keeps its 3px left rail in `--accent-interactive`. **Expandable content collapses too:** dropdowns, expandable nav groups, and the matter/status card must never render their expanded panel inside the 72px rail — text wrapping letter-by-letter in the rail is the tell. In the rail they reduce to their icon; invoking one opens its content as a **flyout popover to the right of the rail** (standard popover per `disclosure-surfaces.md`, opens on click, closes on outside click/Escape). Anything that can't reduce to an icon + flyout hides in the rail entirely.
...
**Belongs:** distinct app sections (Dashboard, Reports), tools/reference, an optional "ACCOUNT" section listing Settings (`settings-and-profile.md`), a persistent "active matter" card at top.

**Does NOT belong:** steppers/wizard step lists (see `steppers-and-wizards.md`), running totals or computed values (use sidebar matter card or sticky page summary), status indicators (top bar), **the user's account control** (top-bar session cluster, per `app-shell-and-headers.md`), tab-style content switchers, in-page filters.
...
Sidebar fits within the viewport vertically. If content exceeds height, it scrolls internally. The sidebar bottom is **empty by default** — the account control lives in the top-bar session cluster in every app (`app-shell-and-headers.md`), so nothing is pinned there unless an app has a genuine app-scoped persistent element (e.g. a storage meter) that earns the slot. Same rule in the mobile drawer: the drawer is navigation only; the account stays in the top bar, reachable without opening the drawer.

**Deprecated (v1.11):** pinned account control (avatar + name/role) at the sidebar bottom → the top-bar avatar, all apps. Remove the block; do not leave an empty pinned region with a rule above it.
...
.nav-sidebar .nav-pinned { flex-shrink: 0; border-top: 1px solid rgba(255,255,255,0.08); }   /* only when an app-scoped element earns the slot — never the account */
```
...
- Account control pinned at the sidebar bottom or listed as a nav row — it lives in the top-bar session cluster, every app
