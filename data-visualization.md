...
- **Status text** (KPI deltas, up/down indicators): `--text-success` / `--text-error` — the semantic text tokens from `_core-requirements.md`, AA in both themes. Never alert fills or hand-picked hex as text, and always pair with an arrow/icon, not color alone.

All palettes must pass colorblind simulation (Deuteranopia, Protanopia, Tritanopia).

## Axes, gridlines, legends
Y-axis starts at zero unless explicitly justified. Horizontal gridlines only (1px `--border-light`), no vertical unless time-based. **Bar tracks in a standalone chart** (the empty remainder of a ranked or percentage bar, on a card or a chart surface): `--color-navy-gray-1` light / `--border-light` dark — the track is a hint of the full scale, not a second bar, so it stays as quiet as the gridlines. Do not use `--color-navy-gray-2` here; that darker track is reserved for meters *inside table rows*, where it has to survive the row-hover tint (see *Inline progress & meter tracks*). Tick labels: Sans 12pt `--text-secondary`, locale-aware formatting. Axis titles only when units aren't obvious — sentence case. Legend near the data; for ≤4 series, label directly on the chart.
...
## Structure
Semantic `<table>` with `<thead>`, `<tbody>`, `<tr>`, `<th>`, `<td>`. `<th scope="col">` on column headers. Right-align numbers, left-align text, center icons-only / status badges. Body cells use `--font-sans`; numeral cells carry `mws-data mws-data--num` (mono, tabular, right-aligned) and identifier cells `mws-data mws-data--id` (mono, never wraps — a wrapped ID loses the alignment it exists for). The pagination count and page label are copy and stay Sans. **Never set table data in the serif `--font-mix` (Georgia)** — serif slows scanning and column comparison. Header row: Sans 12pt 600-weight, ALL CAPS, 5% tracking, `--text-secondary`. Row separators: 1px `--border-light`. **No vertical rules** — they fragment the row. Empty cells: em-dash (`—`), never blank.

## Sorting
**Every data column is sortable** — users shouldn't have to guess which headers respond. Click a header to cycle ascending → descending. Active sort: `caret-up`/`caret-down` in `--accent-interactive`; inactive columns show a faint `caret-up-down` indicator (visible on hover) so sortability is discoverable. Set `aria-sort="ascending|descending|none"` on every sortable header. The only exempt columns are non-data ones: the selection-checkbox column and the row-actions column. Sort semantically where text order lies (priority ranks Critical → Low, not alphabetically; dates by value, not label).
...
## Row actions
Rightmost column: `dots-three-vertical` opens a popover (View, Edit, Duplicate, Delete-at-bottom — the destructive item styled per the destructive-menu-item rule in `disclosure-surfaces.md`: last, separated, `--text-error`). Visible on hover (desktop), persistent on touch. Destructive actions require modal confirmation per `disclosure-surfaces.md`. `aria-haspopup="menu"` + `aria-expanded`.
...
## Inline progress & meter tracks
A progress bar or meter inside a row (deal progress, completion %) has a fill and a **track** — and the track must survive the row's interactive states. **The track never shares the color of the row-hover or selection tint it can sit on**: a `--color-navy-gray-1` track inside a row that hovers to `--color-navy-gray-1` vanishes exactly when the user looks at it. Canon for in-row meters: track = `--color-navy-gray-2` (light) / `--border-light` (dark); fill = `--accent-interactive`. This is one step darker than a standalone chart's track (`--color-navy-gray-1`, see *Axes, gridlines, legends*) precisely because the row can hover to `--color-navy-gray-1`. When adding any in-row meter, verify the track is distinguishable in all three states — rest, hover, selected — in both themes.
...
- Serif (`--font-mix`/Georgia) in table cells — data is for scanning; use `--font-sans`, `mws-data--num` for numerals
- Mono on the pagination footer, record count, or any cell that is a sentence — copy is Sans; mono is applied only through `mws-data` (`_core-requirements.md`, rule 7)
- An ID column that wraps (`AIS-` / `00000019` on two lines) — `mws-data--id` is `nowrap`; give the column the width or drop the prefix into the header
- Unsortable data columns — every data column sorts; only checkbox and actions columns are exempt · hide row actions on touch
- An in-row progress/meter track that matches the row-hover or selection tint (the bar disappears on hover)
