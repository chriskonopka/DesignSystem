...
A progress bar or meter inside a row (deal progress, completion %) has a fill and a **track** — and the track must survive the row's interactive states. **The track never shares the color of the row-hover or selection tint it can sit on**: a `--color-navy-gray-1` track inside a row that hovers to `--color-navy-gray-1` vanishes exactly when the user looks at it. Canon: track = `--color-navy-gray-2` (light) / `--border-light` (dark); fill = `--accent-interactive`. When adding any in-row meter, verify the track is distinguishable in all three states — rest, hover, selected — in both themes.
